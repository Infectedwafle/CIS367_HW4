...
class TruncCone {
   ...
   */
  constructor (gl, radiusBottom, radiusTop, height, subDiv, col1, col2) {
    ...
    /* Instead of allocating two separate JS arrays (one for position and one for color),
      ...
      */
    vertices.push(0,0,height); /* top of truncated cone */
    ...
    for (let k = 0; k < subDiv; k++) {
      ...
      let x = radiusTop * Math.cos (angle);
      let y = radiusTop * Math.sin (angle);

      /* the first three floats are 3D (x,y,z) position */
      vertices.push (x, y, height); /* perimeter of top */
      vec3.lerp (randColor, col1, col2, Math.random()); /* linear interpolation between two colors */
    ...
    vertices.push(0,0,0); /* bottom of truncated cone */
    vec3.lerp (randColor, col1, col2, Math.random()); /* linear interpolation between two colors */
    vertices.push(randColor[0], randColor[1], randColor[2]);
    for (let k = 0; k < subDiv; k++) {
      let angle = k * 2 * Math.PI / subDiv;
      let x = radiusBottom * Math.cos (angle);
      let y = radiusBottom * Math.sin (angle);
    ...
    vertices.push (0,0,0); /* center of bottom */
    vec3.lerp (randColor, col1, col2, Math.random()); /* linear interpolation between two colors */
    vertices.push(randColor[0], randColor[1], randColor[2]);
